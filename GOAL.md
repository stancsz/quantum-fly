...
- [x] 建立真实 Git 基线，使源代码、测试、文档和 verifier 可追踪；`.mochu/VERIFIER_BASELINE` 指向存在的 commit。ship gate 不在未明确请求时覆盖 canonical research receipts，验证器生成临时输出时使用隔离目录并清理锁。提交、推送和公开发布仍需用户明确授权。
...
- [x] 建立可用 Git/verifier baseline，并让 ship gate 保持验证隔离。
...
2026-09-12 fresh mapping records 11/11 criteria complete. The authorized implementation baseline is commit `ecef5619c0e6e16913d4f4e36f08a9fdf8cfb3e3`; `.mochu/VERIFIER_BASELINE` points to that commit, and the MIT license decision is recorded. See `docs/VERIFICATION_2026-09-12.md`. Publication, deployment, investment validity, and production readiness remain outside this completion claim.

### Completion summary

The repository now has turnover-correct backtests, bound cache and data provenance, exact MaleCNS validation, a clean-install and optional-quantum path, preregistered classical controls with uncertainty, bounded concurrent runtime behavior, strict numeric and persistence validation, MIT reuse terms, an accepted Git/verifier baseline, and a criterion-level verification record. This completes the local research and reproducibility goal only within its declared boundaries.
