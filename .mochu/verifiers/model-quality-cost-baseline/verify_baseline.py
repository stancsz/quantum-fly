...
RECEIPT = ROOT / "outputs" / "model-quality-cost-baseline.json"


def main() -> int:
    completed = subprocess.run(
        [sys.executable, "-m", "scripts.run_model_quality_cost_baseline"],
        cwd=ROOT,
        capture_output=True,
        text=True,
        encoding="utf-8",
    ...
    if receipt.get("comparison", {}).get("status") != "complete":
        print("matched baseline comparison is incomplete", file=sys.stderr)
        return 1
    for arm in ("candidate", "baseline"):
        calls = receipt.get("arms", {}).get(arm, [])
    ...
    for metric in ("quality_rate", "latency_mean_seconds", "cost_mean"):
        if metric not in receipt["comparison"].get("candidate", {}) or metric not in receipt["comparison"].get("baseline", {}):
            print(f"comparison metric missing: {metric}", file=sys.stderr)
            return 1
    if not receipt["comparison"].get("uncertainty"):
        print("uncertainty summary missing", file=sys.stderr)
        return 1
    if not any("no advantage" in item.lower() for item in receipt.get("limitations", [])):
